...
export function createSaveData() {
    ...
    const saveData = {
        ...
        player: {
            ...
            race: window.player.race || "human",
            type: window.player.type || "neutral",
...
export async function loadGameFromSave(saveData) {
    ...
    try {
        ...
        const { loadRaces } = await import('./race.js');
        ...
        try {
            ...
            // Load all scenarios to get all possible nodes
            try {
                const scenariosManifest = await fetch(`../story-content/${storyFolder}/scenarios/_scenarios.json?v=${Date.now()}`).then(res => {
                    if (!res.ok) {
                        throw new Error(`Failed to fetch scenarios manifest: ${res.status} ${res.statusText}`);
                    }
                    return res.json();
                });
                
                // Load all scenario data and merge nodes
                for (const scenarioInfo of scenariosManifest) {
                    const scenarioResponse = await fetch(`../story-content/${storyFolder}/scenarios/${scenarioInfo.file}?v=${Date.now()}`);
                    if (scenarioResponse.ok) {
                        const scenarioData = await scenarioResponse.json();
                        // Merge scenario nodes into story data
                        if (scenarioData.nodes) {
                            Object.assign(window.storyData, scenarioData.nodes);
                        }
                    }
                }
                console.log("All scenarios loaded, total nodes:", Object.keys(window.storyData).length);
            } catch (scenarioError) {
                console.warn("Could not load all scenarios:", scenarioError);
                // Continue without scenarios - some nodes might not be available
            }
        ...
        console.log("Story data keys:", window.storyData ? Object.keys(window.storyData) : "No story data");
        console.log("Player available:", !!window.player, window.player?.id);
        console.log("Looking for node in storyData:", window.storyData?.[window.currentNode]);
